    ...
    public function store(Request $request)
    {
        ...
        if(isset($data['is_variant'])) {
            foreach ($data['variant_name'] as $key => $variant_name) {
                $lims_variant_data = Variant::firstOrCreate(['name' => strtoupper($data['variant_name'][$key])]);
                $lims_variant_data->name = strtoupper($data['variant_name'][$key]);
                $lims_variant_data->save();
                $lims_product_variant_data = ProductVariant::where('item_code', strtoupper($data['item_code'][$key]))->count();
                if ($lims_product_variant_data == 0) {
                    $lims_product_variant_new = new ProductVariant;
                    $lims_product_variant_new->product_id = $lims_product_data->id;
                    $lims_product_variant_new->variant_id = $lims_variant_data->id;
                    $lims_product_variant_new->position = $key + 1;
                    $lims_product_variant_new->item_code = strtoupper($data['item_code'][$key]);
                    $lims_product_variant_new->variant_color = strtoupper($data['variant_color'][$key]);
                    $lims_product_variant_new->additional_price = $data['additional_price'][$key];
                    $lims_product_variant_new->qty = 0;
                    $lims_product_variant_new->save();
                }
            ...
            if(isset($data['is_variant'])) {
                foreach ($data['variant_name'] as $key => $variant_name) {
                    if($data['product_variant_id'][$key] == 0) {
                        ...
                        $lims_product_variant_data->position = $key + 1;
                        $lims_product_variant_data->item_code = strtoupper($data['item_code'][$key]);
                        $lims_product_variant_data->variant_color = strtoupper($data['variant_color'][$key]);
                        $lims_product_variant_data->additional_price = $data['additional_price'][$key];
                        $lims_product_variant_data->qty = 0;
                        $lims_product_variant_data->save();
                    }
                        ...
                        ProductVariant::find($data['product_variant_id'][$key])->update([
                            'position' => $key+1,
                            'item_code' => strtoupper($data['item_code'][$key]),
                            'variant_color' => strtoupper($data['variant_color'][$key]),
    ...
    public function productWarehouseData($id)
    {
        ...
        if($lims_product_data->is_variant) {
            ...
            foreach ($lims_product_variant_data as $key => $product_variant_data) {
                ...
                if($product_variant_data->qty > 0) {
                    $variant_name[] = $lims_variant_data->name;
                    $variant_qty[] = $product_variant_data->qty;
                    $variant_color[] = $product_variant_data->variant_color;
                }
            }
        } else {
            $variant_name[] = $lims_product_data->name;
            $variant_qty[] = $lims_product_data->qty;
            $variant_color[] = $lims_product_data->variant_color;
        }
        $product_variant_warehouse = [$variant_name, $variant_qty, $variant_color];
