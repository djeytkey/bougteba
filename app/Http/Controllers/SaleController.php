...
class SaleController extends Controller
{
    ...
    {      
        ...
        if($role->hasPermissionTo('sales-index')) {
            ...
            return view('sale.index',compact('starting_date', 'ending_date', 'warehouse_id', 'status_id', 'lims_gift_card_list', 'lims_pos_setting_data', 'lims_account_list', 'lims_warehouse_list', 'all_permission'));
        }
        else
    ...
    {      
        ...
        if($role->hasPermissionTo('sales-index')) {
            ...
            return view('sale.export',compact('starting_date', 'ending_date', 'warehouse_id', 'status_id', 'lims_gift_card_list', 'lims_pos_setting_data', 'lims_account_list', 'lims_warehouse_list', 'all_permission'));
        }
        else
            return redirect()->back()->with('not_permitted', 'Sorry! You are not allowed to access this module');
    }
    ...
    public function saleData(Request $request)
    {
        ...
        if(!empty($sales))
        {
            foreach ($sales as $key=>$sale)
            {
                ...
                $nestedData['address'] = preg_replace('/[\n\r]/', " ", $sale->customer_address);
                $nestedData['notes'] = preg_replace('/[\n\r]/', " ", $sale->sale_note);
                $nestedData['qty'] = $sale->total_qty;
                ...
                if(in_array("sales-gen-invoice", $request['all_permission'])){
                $nestedData['options'] .= '
                                <li>
    ...
    public function create()
    {
        ...
        if($role->hasPermissionTo('sales-add')){
            ...
            return view('sale.create',compact('role', 'lims_city_list', 'lims_customer_list', 'lims_warehouse_list', 'lims_biller_list', 'lims_pos_setting_data', 'lims_tax_list', 'lims_general_setting_data'));
        }
        else
            return redirect()->back()->with('not_permitted', 'Sorry! You are not allowed to access this module');
    }
    ...
    public function store(Request $request)
    {
        ...
        $data['customer_address'] = preg_replace('/[\t]/', " ", $data['customer_address']);
        $data['customer_name'] = preg_replace('/[\t]/', " ", $data['customer_name']);
        $data['sale_note'] = preg_replace('/[\t]/', " ", $data['sale_note']);
        $data['staff_note'] = preg_replace('/[\t]/', " ", $data['staff_note']);
        
        //$data['customer_address'] = nl2br($data['customer_address']);
        ...
        foreach ($product_id as $i => $id) {
            ...
            $original_gain_price += $original_price[$i] * $qty[$i];
        }
        $sale_gain = new Gain();
        $sale_gain->user_id = Auth::id();
        $sale_gain->sale_id = $lims_sale_data->id;
        $sale_gain->total_original_price = $original_gain_price;
        $sale_gain->total_qty = $lims_sale_data->total_qty;
        $sale_gain->total_livraison = $lims_sale_data->livraison;
        $sale_gain->total_discount = $lims_sale_data->total_discount;
        $sale_gain->grand_total = $lims_sale_data->grand_total;
        $sale_gain->save();
    ...
    public function limsProductSearch(Request $request)
    {
        ...
        $product[] = User::find(Auth::user()->id)->discount;
        return $product;
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        foreach ($product_id as $key => $pro_id) {
            ...
            $original_gain_price += $original_price[$key] * $qty[$key];
        }
        ...
        $lims_gain_data = Gain::where('sale_id', $lims_sale_data->id)->first();
        $lims_gain_data->total_original_price = $original_gain_price;
        $lims_gain_data->total_qty = $lims_sale_data->total_qty;
        $lims_gain_data->total_livraison = $lims_sale_data->livraison;
        $lims_gain_data->total_discount = $lims_sale_data->total_discount;
        $lims_gain_data->grand_total = $lims_sale_data->grand_total;
        $lims_gain_data->save();
        //$lims_customer_data = Customer::find($data['customer_id']);
    ...
    public function addDiscount(Request $request)
    {
        // dd($request);
        $data = $request->all();
        $lims_sales_data = Sale::where('reference_no', $data['discount_reference_no'])->first();
        $lims_sales_data->total_discount = $data['total_discount'];
        $lims_sales_data->save();
        $lims_gain_data = Gain::where('sale_id', $lims_sales_data->id)->first();
        $lims_gain_data->total_discount = $data['total_discount'];
        $lims_gain_data->save();

        $message = "Remise ajoutée avec succée !";
    ...
    public function exportBySelection(Request $request)
    {
        $sale_id = $request['saleIdArray'];
        foreach ($sale_id as $id) {
            $lims_sale_data = Sale::find($id);
            if ($lims_sale_data->is_valide) {
                $lims_sale_data->export = 1;
                $lims_sale_data->save();
            }
        }
        $message = 'Sale ready to export!';
        return redirect('sales')->with('message', $message);
    }

    public function returnexportBySelection(Request $request)
    {
        $sale_id = $request['saleIdArray'];
        foreach ($sale_id as $id) {
            $lims_sale_data = Sale::find($id);
            $lims_sale_data->export = NULL;
            $lims_sale_data->save();
        }
        $message = 'Exports returned successfully!';
        return redirect('sales')->with('message', $message);
    }

    public function senddeliveryBySelection(Request $request)
    {
        $sale_id = $request['saleIdArray'];
        foreach ($sale_id as $id) {
            $lims_sale_data = Sale::find($id);
            $lims_sale_data->delivery_status = 1;
            $lims_sale_data->save();

            $delivery = new Delivery();
            $delivery->reference_no = 'dr-' . date("dmy") . '-'. date("His");
            $delivery->sale_id = $lims_sale_data->id;
            $delivery->user_id = Auth::id();
            $delivery->sold_by = $lims_sale_data->user_id;
            $delivery->delivered_by = '';
            $delivery->note = preg_replace('/[\n\r]/', " ", $lims_sale_data->sale_note);
            $delivery->is_close = 0;
            $delivery->returned = 0;
            $delivery->save();

            $deliveries = new DeliveryStatus();
            $deliveries->reference_no = $delivery->reference_no;
            $deliveries->status = 1;
            $deliveries->status_date = date("d-m-Y") . '-'. date("H:i:s");
            $deliveries->save();
            sleep(1);
        }
